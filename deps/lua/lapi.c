...
lua_initsharedproto(lua_State *L, Proto *p, const TValue *val, const TValue *gt) {
	...
	luaF_initupvals(L, pcl);
    setobj(L, pcl->upvals[0]->v, gt);
	luaC_upvalbarrier(L, pcl->upvals[0]);

	setclLvalue(L, &cl, pcl);
	setobj2t(L, luaH_set(L, hvalue(gt), &cl), val);
	invalidateTMcache(hvalue(gt));

	for (i = 0; i < p->sp->sizep; ++i) {
		lua_initsharedproto(L, p->p[i], val, gt);
	}
}

LUA_API void* lua_initsharedclosure(lua_State *L) {
	int i;
	TValue *cl;
...
static void
lua_deinitsharedproto(lua_State *L, Proto *p, const TValue *gt) {
	int i;
	TValue cl;
	setclLvalue(L, &cl, cast(LClosure *, p->sp->closure));
	setobj2t(L, luaH_set(L, hvalue(gt), &cl), luaO_nilobject);
	invalidateTMcache(hvalue(gt));

	p->sp->refnum = 0;
	p->sp->closure = NULL;
	for (i = 0; i < p->sp->sizep; ++i) {
		lua_deinitsharedproto(L, p->p[i], gt);
	}
}

LUA_API void lua_deinitsharedclosure(lua_State *L, void *f) {
	LClosure *pcl;
	Table *reg;
	const TValue *gt;
	...
	reg = hvalue(&G(L)->l_registry);
	gt = luaH_getint(reg, LUA_RIDX_GLOBALS);
	lua_deinitsharedproto(L, pcl->p, gt);
	
	/* maybe TO BE FIXED! gc call. */
	lua_unlock(L);
}

LUA_API void lua_handlesharedclosure(void *f) {
	SharedProto *sp = (cast(LClosure *, f))->p->sp;
	atomic_inc(sp->refnum);
}

LUA_API void lua_releasesharedclosure(void *f) {
	LClosure *pcl = cast(LClosure *, f);
	SharedProto *sp = pcl->p->sp;
	if ((atomic_dec(sp->refnum)) == 1) {
		Table *reg;
		const TValue *gt;
		TValue cl;
		global_State *gstate = cast(global_State *, sp->l_G);
		atomic_t *lockptr = cast(atomic_t *, gstate->ud);
		lua_State *L = gstate->mainthread;
		spin_lock_ptr(lockptr);
		lua_lock(L);

		reg = hvalue(&(gstate->l_registry));
		gt = luaH_getint(reg, LUA_RIDX_GLOBALS);
		setclLvalue(L, &cl, cast(LClosure *, pcl));
		setobj2t(L, luaH_set(L, hvalue(gt), &cl), luaO_nilobject);
		invalidateTMcache(hvalue(gt));

		/* maybe TO BE FIXED! gc call. */
		lua_unlock(L);
		spin_unlock_ptr(lockptr);
	}
}

static void
lua_refsharedproto(Proto *p) {
	int i;
	atomic_inc(p->sp->refnum);
	for (i = 0; i < p->sp->sizep; ++i) {
		lua_refsharedproto(p->p[i]);
	}
}

LUA_API void lua_refsharedclosure(void *f) {
	if (f == NULL) return;
	lua_refsharedproto((cast(LClosure *, f))->p);
}

LUA_API void lua_unrefsharedclosure(void *f) {
	int i;
	Proto *p = cast(LClosure *, f)->p;
	for (i = 0; i < p->sp->sizep; ++i) {
		lua_unrefsharedclosure(p->p[i]->sp->closure);
	}
	lua_releasesharedclosure(f);
}
#endif
