...
bson_reserve(struct bson *b, int sz) {
	...
	if (b->ptr == b->buffer) {
		b->ptr = (uint8_t *)nl_malloc(b->cap);
		memcpy(b->ptr, b->buffer, b->size);
	} else {
		b->ptr = (uint8_t *)nl_realloc(b->ptr, b->cap);
	}
}
...
typeclosure(lua_State *L) {
	static const char * typename_array[] = {
		"number",	// 1
		"boolean",	// 2
		"table",	// 3
		"nil",		// 4
	...
	int i;
	int n = sizeof(typename_array) / sizeof(typename_array[0]);
	for (i=0;i<n;i++) {
		lua_pushstring(L, typename_array[i]);
	}
	lua_pushcclosure(L, ltype, n);
}

static uint8_t oid_header[5];
static uint32_t oid_counter = 0;
...
lobjectid(lua_State *L) {
	...
	if (lua_isstring(L,1)) {
		...
		uint32_t id = atomic_inc(oid_counter);
...
static atomic_t g_inited = 0;

static void init_once() {
	if (g_inited == 0 && atomic_cas(g_inited, 0, 1)) {
		init_oid_header();
		init_bson_numstr();
	}
}
